...
요청량 (⚠️ 2026-08-05 실측)
    ~50요청/28초를 넣었더니 HTTP 429 TOO_MANY_REQUESTS 가 걸렸고 **3시간 41분이
    지나도** 풀리지 않았다(그 중 마지막 2시간은 요청을 한 번도 넣지 않았는데도 그랬다).
    IP 단위이고 호스트 공통이라 new.land 까지 함께 막힌다 — Playwright 로 되돌려도
    안 풀린다. 반면 웹페이지(HTML)는 200 이다. 즉 API 만 막는다.

    한 번 걸리면 초·분 단위로는 복구되지 않는다. 그래서 이 클라이언트는 재시도로
    뚫는 대신 **요청 수 자체를 줄이는** 쪽으로 설계했다.
